...
probe_nclasses = args.probe_nclasses
...
num_samples_to_show = int(probe_nclasses//cols)


print('CIAO?', len(class_images[1]))  # Length of the list or number of keys
print(type(class_images), type(class_images[0]))  # Type of the first element
print('chiavi',len(class_images.keys()))
            ...
            with col.select((col.extent[0]+text_width+bpad, *col.extent[1:])):
                ...
                for row_idx, row in enumerate(subgrid):
                    for col_idx, cell in enumerate(row):
                        ...
                        key_index = grid_idx + fig_idx * cols
                        if key_index < len(class_keys):
                            key = class_keys[key_index]
                            img_idx = row_idx * subcols + col_idx
                            if img_idx < len(class_images[key]):
                                cell.imshow(class_images[key][img_idx].clip(0, 1))
                        else:
                            print(key_index)
