...
def filter_artists(use_artists):
    ...
    final_concept_names['Repetitions'] = final_concept_names.groupby('Word')['Word'].transform('count')
    final_concept_names = final_concept_names.sort_values(by='Repetitions', ascending=False).drop_duplicates(subset='Word', keep='first')
...
def main(args):
    ...
    if args.use_wandb:
        ...
        wandb.init(
            project="Train linear probe - with test",
            entity="edutc200-uva",
    ...
    # Filter out artists
    neuron_list = filter_artists(use_artists=True)
    neuron_list = [n - 1 for n in neuron_list]

    # Keep only the columns corresponding to indices in neuron_list
    train_data = train_data[:, neuron_list]
    train_val_data = train_val_data[:, neuron_list]
    test_data = test_data[:, neuron_list]

    # Redefine num concepts
    num_concepts = (
        args.autoencoder_input_dim_dict[args.ae_input_dim_dict_key[args.modality]] * args.expansion_factor
        - len(neuron_list)
    )
    print('Using', num_concepts, 'neurons. Removed ', len(neuron_list), 'concepts.')
        ...
        if (e+1) % args.probe_val_freq == 0:
            ...
            if args.use_wandb:
                ...
                postfix_list = ['Total Loss', 'Classification Loss', 'Sparsity Loss',
                                'VAL accuracy top1', 'VAL accuracy top5', 'TEST accuracy top1', 'TEST accuracy top5']
                stat_list = [val_loss, val_class_loss,
                             val_sparse_loss, val_acc_top1, val_acc_top5, test_acc_top1, test_acc_top5]
