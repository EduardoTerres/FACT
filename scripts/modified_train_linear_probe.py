...
# Precompute related concepts and their similarities for each class
def precompute_related_concepts(closest_concepts, num_concepts):
    """
    Precompute a mask for related concepts and their similarity scores.

    Args:
        closest_concepts (dict): Dictionary mapping class names to related concepts and their similarities.
        num_concepts (int): Total number of concepts.

    Returns:
        related_masks (torch.Tensor): Binary mask of shape [num_classes, num_concepts].
        similarity_matrix (torch.Tensor): Similarity scores of shape [num_classes, num_concepts].
    """
    num_classes = len(closest_concepts)
    related_masks = torch.zeros((num_classes, num_concepts), dtype=torch.bool)
    similarity_matrix = torch.zeros((num_classes, num_concepts), dtype=torch.float32)

    for class_idx, (class_name, concepts_info) in enumerate(closest_concepts.items()):
        for concept_idx, _, similarity in concepts_info:
            related_masks[class_idx, concept_idx] = 1
            similarity_matrix[class_idx, concept_idx] = similarity

    return related_masks, similarity_matrix

def compute_penalty_loss(model, related_masks, similarity_matrix, train_y, penalty_lambda, device):
    """
    Efficiently compute the penalty loss per class using matrix operations.

    Args:
        model (torch.nn.Module): Linear model with weights.
        related_masks (torch.Tensor): Binary mask of shape [num_classes, num_concepts].
        similarity_matrix (torch.Tensor): Similarity scores of shape [num_classes, num_concepts].
        train_y (torch.Tensor): Batch of class indices (shape [batch_size]).
        penalty_lambda (float): Regularization hyperparameter.
        device (torch.device): Device to perform computations on (CPU or GPU).

    Returns:
        penalty_loss (torch.Tensor): Computed penalty loss for the batch.
    """
    related_masks = related_masks.to(device)
    similarity_matrix = similarity_matrix.to(device)
    weights = model.weight.to(device)  # [num_classes, num_concepts]
    
    batch_related = related_masks[train_y]  # [batch_size, num_concepts]
    batch_sim = similarity_matrix[train_y]  # [batch_size, num_concepts]
    batch_weights = weights[train_y]  # [batch_size, num_concepts]

    # 1. Penalize low similarity weights
    low_sim_penalty = (batch_weights.abs() * batch_related * (1 - batch_sim)).sum()
    
    # 2. Encourage alignment via cosine similarity
    weights_norm = F.normalize(batch_weights, p=2, dim=1)
    sim_norm = F.normalize(batch_sim, p=2, dim=1)
    cosine_loss = - (weights_norm * sim_norm).sum(dim=1).mean()  # Negative to maximize
    
    # 3. Enforce positive weights for related concepts
    positivity_penalty = F.relu(-batch_weights * batch_related).sum()

    # Combine terms
    total_penalty = penalty_lambda * (low_sim_penalty + positivity_penalty) + penalty_lambda * cosine_loss
...
def main(args):
    ...
    if args.probe_on_features:
        ...
        print ("Training probe on features")
    else:
        model = torch.nn.Linear(
            num_concepts, num_classes, bias=False)
        print ("Training probe on concepts")
    ...
    related_masks = related_masks.to(args.device)
    similarity_matrix = similarity_matrix.to(args.device)
    ...
    for e in range(args.probe_epochs):
        ...
        for batch_idx, (train_X, train_y) in enumerate(tqdm(train_loader)):
            ...
            # Penalty loss
            penalty_loss = compute_penalty_loss(model, related_masks, similarity_matrix, train_y, args.penalty_lambda, args.device)
            total_penalty_loss += penalty_loss.item()

            # Total loss
            total_classification_loss += classification_loss.item()
            loss = classification_loss + args.probe_sparsity_loss_lambda * sparsity_loss + penalty_loss
            total_loss += loss.item()
        ...
        avg_class_loss = total_classification_loss / total_batches
        avg_sparse_loss = total_sparsity_loss / total_batches
        avg_penalty_loss = total_penalty_loss / total_batches

        print(f"Epoch: {e+1}, Training Loss: {avg_loss:.4f}, Classification Loss: {avg_class_loss:.4f}, "
            f"Sparsity Loss: {avg_sparse_loss:.4f}, Penalty Loss: {avg_penalty_loss:.4f}")
        
        if args.use_wandb:
            location = MetricLocation.TRAIN
            name = 'statistics'
            postfixes = ['Total Loss', 'Classification Loss', 'Sparsity Loss', 'Penalty Loss']
            metric_vals = [avg_loss, avg_class_loss, avg_sparse_loss, avg_penalty_loss]  # Include avg_penalty_loss here
            metrics = []
    ...
    torch.save(saving_dict_final, osp.join(checkpoint_save_path,
                                           f"modified_{prefix}_final_{args.probe_config_name}.pt"))
    ...
    print (f"Test accuracy top1: {test_acc_top1}, Test accuracy top5: {test_acc_top5}")
