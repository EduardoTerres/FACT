...
def filter_based_on_alignment(p_1, p_2):
    ...
    p1_index = int(len(final_concept_names) * p_1)
    p2_index = int(len(final_concept_names) * p_2)
    selected_concepts = final_concept_names.iloc[p1_index:p2_index]['Neuron'].values

    return [neuron for neuron in range(1, 8192) if neuron not in selected_concepts]
...
def main(args, results):
    ...
    for p in [0, 0.2, 0.4, 0.6, 0.8]:
        ...
        mask = filter_based_on_alignment(p, p + 0.2)

        test_data_copy[:, mask] = 0

        print('Masked neurons: ', len(mask))

        # Create dataloader
        test_data_copy = TensorDataset(test_data_copy, test_labels_copy)
        test_loader_copy = torch.utils.data.DataLoader(
            test_data_copy, batch_size=args.probe_train_bs, shuffle=False)
        
        # Evaluation
        _, _, _, acc_top1, acc_top5, _, _ = eval_model(
            model, test_loader_copy, num_classes, args.probe_classification_loss,
            args.probe_sparsity_loss_lambda, args.device, eval_coverage=True,
        )

        acc_top1 = acc_top1.item()
        acc_top5 = acc_top5.item()
        results.append([p, acc_top1, acc_top5])
...
results_df = pd.DataFrame(results, columns=['p', 'acc_top1', 'acc_top5'])
results_df.to_csv(f'./analysis/alignment_ablation_{args.probe_dataset}.csv', index=False)
