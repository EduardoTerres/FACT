...
class MethodOurs(MethodBase):
    ...
    def __init__(self, args, vocab_txt_path=None, embeddings_path=None, use_fixed_sae=False, use_sae_from_args=True, **kwargs):
        self.args = args
        ...
        if use_fixed_sae:
            ...
            self.probe_config_dict = {'imagenet': "lr0.001_bs512_epo200_nobias_clCE_spL1_spl1.0",
                                      ...
                                      'waterbirds': 'lr0.1_bs512_epo200_nobias_clCE_spL1_spl10.0',
                                      'full-imagenet': "lr0.001_bs512_epo200_nobias_clCE_spL1_spl1.0" }
        ...
        if embeddings_path is not None:
            ...
            if type(embeddings_path) == str:
                ...
                self.vocab_txt_all = [np.genfromtxt(
                    t, dtype=str, delimiter='\n', autostrip=False) for t in vocab_txt_path]
        else:
    ...
    def get_top_k_closest_concepts(self, word_embeddings, k=5):
        """
        Find the top-k closest concepts to given word embeddings.

        Args:
            word_embeddings (torch.Tensor): Embeddings of the words from the text file (shape: [num_words, embedding_dim]).
            k (int): Number of closest concepts to find.

        Returns:
            closest_concepts (list): List of top-k concept names for each word embedding.
            similarities (list): List of top-k similarity scores for each word embedding.
            indices (list): List of top-k concept indices for each word embedding.
        """
        closest_concepts = []
        similarities = []
        indices = []
        ...
        dic_vec = self.all_dic_vec  # Shape: [embedding_dim, num_concepts]
        dic_vec /= dic_vec.norm(dim=0, keepdim=True)  # Normalize concept vectors

        for word_embedding in word_embeddings:  # Iterate through word embeddings
            word_embedding /= word_embedding.norm()  # Normalize the word embedding
            sims = torch.matmul(dic_vec.T, word_embedding)  # Compute cosine similarity
            top_k = torch.topk(sims, k=k, dim=0)  # Get top-k similarities and indices

            indices.append(top_k.indices.tolist())
            similarities.append(top_k.values.tolist())
            closest_concepts.append([self.vocab_txt_all[0][idx] for idx in top_k.indices])

        return closest_concepts, similarities, indices


def get_method(method, args, **kwargs):
    if method == "ours":
        return MethodOurs(args, use_fixed_sae=True, use_sae_from_args=False, **kwargs)
    else:
